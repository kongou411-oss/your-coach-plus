...
# -*- coding: utf-8 -*-
"""
主食のビタミン・ミネラルデータを一括更新するスクリプト
野菜の更新スクリプトと完全に同じロジックを使用
"""
...
def update_staples_data(file_path):
    """主食のビタミン・ミネラルデータを更新"""

    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    for staple_name, nutrients in STAPLES_DATA.items():
        # 食品エントリを検索（1行形式）
        pattern = rf'(\s*"{re.escape(staple_name)}":\s*\{{[^}}]+\}}),?'

        match = re.search(pattern, content)
        if not match:
            print(f"[WARN] {staple_name} が見つかりませんでした")
            continue
        ...
        try:
            ...
            json_str = old_line.replace(f'"{staple_name}":', '').strip()
            ...
            print(f"[WARN] {staple_name} のパースに失敗: {e}")
            existing_data = {}
        ...
        vitamin_mapping = {
            ...
            'folate': 'folate',  # JSONでは"folate"、野菜では"folicAcid"
        ...
        new_line = f'"{staple_name}": {{ {", ".join(fields)} }}'

        # 置換
        content = content.replace(old_line, new_line)
        print(f"[OK] {staple_name} を更新しました")

    # ファイルに書き込み
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)

    print(f"\n[OK] 完了: {len(STAPLES_DATA)}件の主食を更新しました")
...
if __name__ == '__main__':
    ...
    import subprocess
    print("Gitで元に戻しています...")
    subprocess.run(['git', 'checkout', 'src/foodDatabase.js'], check=True)

    # 更新を実行
    update_staples_data(file_path)
