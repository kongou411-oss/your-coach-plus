...
# -*- coding: utf-8 -*-
"""
調味料9件のビタミン・ミネラルデータを一括更新するスクリプト
野菜・主食・果物・豆類の更新スクリプトと完全に同じロジックを使用
"""
...
# 調味料8件のビタミン・ミネラルデータ
with open('seasonings_vitamins_minerals.json', 'r', encoding='utf-8') as f:
    SEASONINGS_DATA = json.load(f)

def update_seasonings_data(file_path):
    """調味料のビタミン・ミネラルデータを更新"""

    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    for item_name, nutrients in SEASONINGS_DATA.items():
        # 食品エントリを検索（1行形式）
        ...
        # その他の情報を保持（調味料には脂肪酸、omega3がある可能性）
        for key in ['saturatedFat', 'monounsaturatedFat', 'polyunsaturatedFat', 'omega3']:
            if key in existing_data:
                new_data[key] = existing_data[key]
    ...
    print(f"\n[OK] 完了: {len(SEASONINGS_DATA)}件の調味料を更新しました（ピンクソルトの全ビタミン・ミネラルデータ登録完了）")
...
if __name__ == '__main__':
    ...
    # 更新を実行
    update_seasonings_data(file_path)
