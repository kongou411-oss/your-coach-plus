import foodDatabase from './src/foodDatabase.js';

// すべての食品を配列化
const foods = [];
Object.values(foodDatabase.default || foodDatabase).forEach(category => {
...
fattyAcids.forEach(fa => {
  console.log(`\n${fa}が登録されている食品例（値>0）:`);
  const samples = foods.filter(f => f[fa] > 0).slice(0, 5);
  samples.forEach(s => {
