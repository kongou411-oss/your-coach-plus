...
# -*- coding: utf-8 -*-
"""
豆類・ナッツ12件のビタミン・ミネラルデータを一括更新するスクリプト
野菜・主食・果物の更新スクリプトと完全に同じロジックを使用
"""

import re
import json

# 豆類・ナッツ12件のビタミン・ミネラルデータ
with open('beans_nuts_vitamins_minerals.json', 'r', encoding='utf-8') as f:
    BEANS_NUTS_DATA = json.load(f)

def update_beans_nuts_data(file_path):
    """豆類・ナッツのビタミン・ミネラルデータを更新"""

    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    for item_name, nutrients in BEANS_NUTS_DATA.items():
        # 食品エントリを検索（1行形式）
        ...
        # その他の情報を保持（豆類・ナッツにはaminoAcidScore, pdcaas, diaas, dit, 脂肪酸がある）
        for key in ['aminoAcidScore', 'pdcaas', 'diaas', 'dit', 'gi',
    ...
    print(f"\n[OK] 完了: {len(BEANS_NUTS_DATA)}件の豆類・ナッツを更新しました")
...
if __name__ == '__main__':
    ...
    # 更新を実行
    update_beans_nuts_data(file_path)
