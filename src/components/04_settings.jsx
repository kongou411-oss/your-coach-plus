                    ...
                    <details className="border rounded-lg">
                        ...
                        <div className="space-y-4">
                            ...
                            {(() => {
                                ...
                                const [recalcProgress, setRecalcProgress] = React.useState({ current: 0, total: 0 });
                                ...
                                const recalculateAllScores = async () => {
                                    ...
                                    try {
                                        ...
                                        // 登録日から現在までの日付リストを生成
                                        const registrationDate = profile.registrationDate || profile.createdAt || profile.joinDate;
                                        let startDate;
                                        if (registrationDate) {
                                        ...
                                        setRecalcProgress({ current: 0, total: dates.length });

                                        // targetPFCを計算
                                        const targetPFC = PFCUtils.calculateTargetPFC(
                                            profile.weight,
                                        ...
                                        for (let i = 0; i < dates.length; i++) {
                                            const date = dates[i];
                                            const record = await DataService.getDailyRecord(userId, date);

                                            if (record && (record.meals?.length > 0 || record.workouts?.length > 0 || record.conditions)) {
                                                // スコアを計算
                                                const scores = DataService.calculateScores(profile, record, targetPFC);
                                                ...
                                                await DataService.saveDailyRecord(userId, date, record);
                                                updatedCount++;
                                            }

                                            setRecalcProgress({ current: i + 1, total: dates.length });
                                        }

                                        toast.success(`${updatedCount}日分のスコアを再計算しました`);
                                        ...
                                        console.error('[Settings] スコア再計算エラー:', error);
                                ...
                                return (
                                    <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
                                        <h4 className="font-bold mb-2 text-purple-800 flex items-center gap-2">
                                            <Icon name="Calculator" size={16} />
                                            スコア再計算
                                        </h4>
                                        <p className="text-sm text-gray-600 mb-3">
                                            登録日から現在までの全データのスコア（食事・運動・コンディション）を再計算します。
                                            履歴グラフでスコアが表示されない場合に実行してください。
                                        </p>
                                        {recalculating && (
                                            <div className="mb-3 p-3 bg-white rounded-lg border">
                                                <div className="flex items-center justify-between mb-2">
                                                    <span className="text-sm font-medium text-gray-700">再計算中...</span>
                                        ...
                                        <button
                                            ...
                                            className="w-full px-4 py-3 bg-purple-600 text-white font-bold rounded-lg hover:bg-purple-700 transition flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            <Icon name="RefreshCw" size={16} className={recalculating ? 'animate-spin' : ''} />
                                            {recalculating ? 'スコア再計算中...' : 'スコアを再計算する'}
                                        </button>
                                    </div>
                                );
                            })()}
